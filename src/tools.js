...
function track(eventName, metadata={}) {
  if (activeToolSession && metadata.tool) {
    if (eventName === 'tool_interaction_started') activeToolSession.interacted = true;
    if (eventName === 'tool_completed') activeToolSession.completed = true;
    if (eventName === 'tool_exported') activeToolSession.exported = true;
  }
...
function routeTool() {
  ...
  document.documentElement.classList.remove('route-pending');
  activeToolSession = { tool:key, path:location.pathname, openedAt:Date.now(), interacted:false, completed:false, exported:false };
  track('tool_opened', { tool:key, path:location.pathname, sessionId:presenceSessionId });
  $(`[data-tool-panel="${key}"]`)?.addEventListener('input',()=>{
    if(activeToolSession?.tool===key&&!activeToolSession.interacted){
      track('tool_interaction_started',{tool:key,path:location.pathname,sessionId:presenceSessionId});
    }
  },{once:true});
}
...
function downloadTimesheet() {
  calculateTime({silent:true});
  const rows = [['Day','Clock in','Clock out','Break minutes','Hours'],...lastTimeResult.entries.map(item=>[item.day,item.start,item.end,item.breakMinutes,(item.minutes/60).toFixed(2)]),[],['Regular hours',(lastTimeResult.regularMinutes/60).toFixed(2)],['Overtime hours',(lastTimeResult.overtimeMinutes/60).toFixed(2)],['Estimated gross pay',lastTimeResult.pay.toFixed(2)],[],['Created with Searya','https://searya.com/time-card-calculator']];
...
function initializeAuth(){
  ...
  $('#login-form').addEventListener('submit',async event=>{event.preventDefault();const button=event.submitter;button.disabled=true;try{const values=Object.fromEntries(new FormData(event.currentTarget));await SearyaApi.login(values);const session=await SearyaApi.me();if(!session?.user)throw new ApiError('Your sign-in could not be saved. Enable cookies and try again.',401,'SESSION_NOT_SAVED');currentUser=session.user;closeAuth();await refreshAccount();showToast(`Signed in successfully. Welcome back${currentUser?.name?`, ${currentUser.name.split(' ')[0]}`:''}!`);track('auth_completed',{mode:'login'});if(currentUser.isAdmin)location.assign('/admin.html');else await openAccount();}catch(error){$('#auth-message').textContent=error.message;track('auth_failed',{mode:'login',code:error.code});}finally{button.disabled=false;}});
  ...
  const registrationForm=$('#register-form');if(registrationForm&&!registrationForm.elements.companyWebsite)registrationForm.insertAdjacentHTML('afterbegin','<input name="companyWebsite" type="text" tabindex="-1" autocomplete="off" aria-hidden="true" style="position:absolute;left:-10000px;width:1px;height:1px;opacity:0">');
  registrationForm.addEventListener('submit',async event=>{event.preventDefault();const button=event.submitter;button.disabled=true;try{const values=Object.fromEntries(new FormData(event.currentTarget));const result=await SearyaApi.register({...values,role:'both'});if(result.verificationRequired){authTab('login');const message=$('#auth-message');message.textContent='Account created successfully. Check your inbox, verify your email, then sign in.';message.classList.add('success');showToast('Account created successfully. Please verify your email.');}else{currentUser=result.user;closeAuth();await refreshAccount();showToast('Account created successfully. Your workspace is ready.');}track('auth_completed',{mode:'register'});}catch(error){$('#auth-message').classList.remove('success');$('#auth-message').textContent=error.message;track('auth_failed',{mode:'register',code:error.code});}finally{button.disabled=false;}});
...
async function initializeTelemetry(){
  try{await SearyaApi.trackPageView(`${location.pathname}${location.search}`,document.referrer);await SearyaApi.trackPresence(presenceSessionId,'enter',location.pathname,innerWidth<640?'mobile':innerWidth<1000?'tablet':'desktop');presenceTimer=setInterval(()=>SearyaApi.trackPresence(presenceSessionId,'heartbeat',location.pathname,innerWidth<640?'mobile':innerWidth<1000?'tablet':'desktop').catch(()=>{}),30000);addEventListener('pagehide',()=>{clearInterval(presenceTimer);if(activeToolSession?.interacted&&!activeToolSession.completed&&!activeToolSession.exported){navigator.sendBeacon?.('/api/analytics/event',new Blob([JSON.stringify({eventName:'tool_abandoned',metadata:{tool:activeToolSession.tool,path:activeToolSession.path,sessionId:presenceSessionId,durationSeconds:Math.round((Date.now()-activeToolSession.openedAt)/1000)}})],{type:'application/json'}));}navigator.sendBeacon?.('/api/analytics/presence',new Blob([JSON.stringify({sessionId:presenceSessionId,action:'leave',path:location.pathname,device:innerWidth<640?'mobile':'desktop'})],{type:'application/json'}));});}catch{}
}
